...
import time
import pickle
from modes import mode_switcher
...
key = 3 for controller data, key = 4 for program switch notification
"""
...
def parse_controller_data(data):
    ...
    with controller_lock:
        controller_data = pickle.loads(data)  # Assuming data is pickled controller state

def get_controller_data():
    with controller_lock:
        return controller_data

def parse_program_switch(data):
    mode_num = struct.unpack("!i", data)[0]  # Assuming mode number is an integer
    print(f"Received program switch notification: mode {mode_num}")
    mode_switcher.switch_mode(mode_num)
    ...
    while not stop_event.is_set() and connection_alive:
        ...
        while True:
            ...
            if key == 3:
                ...
                print(f"Received controller data: {len(msg_data)} bytes")
            elif key == 4:
                parse_program_switch(msg_data)
            else:
