...
def get_controller_state():
    ...
    controller_state = {}
    for name1, value1 in xi.GetState(0)[1]._fields_:
        if name1 == "wButtons":
            state = xi.GetState(0)[1]  # XINPUT_GAMEPAD instance
            buttons = state.wButtons  # XINPUT_BUTTONS instance

            # Print button states directly from the structure fields
            for name, _, _ in buttons._fields_:
                if "reserved" not in name:  # Skip reserved fields
                    pressed = getattr(buttons, name)
                    controller_state[name] = 1 if pressed else 0
        else:
            controller_state[name1] = getattr(xi.GetState(0)[1], name1)
...
if __name__ == "__main__":
    ...
    #                     pressed = getattr(buttons, name)
    #                     controller_state[name] = 1 if pressed else 0
    #                     # print(f"{name}: {1 if pressed else 0}")
    #         else:
    #             controller_state[name1] = getattr(xi.GetState(0)[1], name1)
    #             # print(f"{name1}: {getattr(xi.GetState(0)[1], name1)}")
    #
    #     print(f"\r{controller_state}", end="")
    #     sleep(0.05)
    while True:
        print(f"\r{get_controller_state()}", end="")
        sleep(0.05)